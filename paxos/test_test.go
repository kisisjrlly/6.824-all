...
func ndecided(t *testing.T, pxa []*Paxos, seq int) int {
	Debug(0, "decided test begin with seq:", seq)
	count := 0
	var v interface{}
	for i := 0; i < len(pxa); i++ {
		if pxa[i] != nil {
			decided, v1 := pxa[i].Status(seq)
			Debug(0, "peer ", i, " has decide seq ", seq, " to be:", decided, v1)
			if decided {
				if count > 0 && v != v1 {
...
func TestDeaf(t *testing.T) {
	// the deaf node can not listen but can ask, but basic paxos does not condider this situation
	// basic paxos does not indicate when prepar been rejected, what the deaf should do
	runtime.GOMAXPROCS(4)
	...
	fmt.Println("two deaf but test pass")

	fmt.Println("one deaf come back")
	pxa[0].Start(1, "xxx")
	...
	fmt.Println("one deaf come back success")
...
func TestForget(t *testing.T) {
	...
	fmt.Println("min init success")
	pxa[0].Start(0, "00")
	pxa[1].Start(1, "11")
	pxa[2].Start(2, "22")
	pxa[0].Start(6, "66")
	pxa[1].Start(7, "77")
	fmt.Println("start finished,wait for success")
	waitn(t, pxa, 0, npaxos)
	fmt.Println("0 start success")
	// Min() correct?
	for i := 0; i < npaxos; i++ {
		m := pxa[i].Min()
		fmt.Println("after paxos,first test min of every peer is:", i, m)
		if m != 0 {
	...
	waitn(t, pxa, 1, npaxos)
	fmt.Println("1 start success")
	// Min() correct?
	for i := 0; i < npaxos; i++ {
		m := pxa[i].Min()
		fmt.Println("after paxos,second test min of every peer is:", i, m)
		if m != 0 {
			t.Fatalf("wrong Min() %v; expected 0", m)
		}
	}
	...
	fmt.Println("all peer done 0 finished")
	for i := 1; i < npaxos; i++ {
		pxa[i].Done(1)
	}
	fmt.Println("all peer done 1 finished")
	for i := 0; i < npaxos; i++ {
		pxa[i].Start(8+i, "xx")
	}
	fmt.Println("all peer start from 8 finished")
	allok := false
...
func TestForgetMem(t *testing.T) {
	...
	pxa[0].Start(0, "x")
	waitn(t, pxa, 0, npaxos)
	var m0_0 runtime.MemStats
	runtime.ReadMemStats(&m0_0)
	fmt.Println("m0_0 alloc:", m0_0.Alloc)
...
func TestMServer(t *testing.T) {
	...
	for seq := 1; seq < ninst; seq++ {
		...
		for seq >= 5 && ndecided(t, pxa, seq-5) < npaxos {
			fmt.Println("continue to detect the paxos")
